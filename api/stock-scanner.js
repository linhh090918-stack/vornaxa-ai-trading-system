...
export default async function handler(req, res) {
  ...
  if (!token) {
    return res.status(500).json({
      error: 'FINNHUB_API_KEY is missing. Add it in Vercel Project Settings → Environment Variables, then redeploy.'
    });
  }

  try {
    const results = await Promise.all(
      STOCK_UNIVERSE.map(async (item) => {
    ...
    return res.status(500).json({
      error: error.message || 'Failed to fetch Finnhub market data.'
    });
  }
}
